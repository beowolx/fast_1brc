mod mapping {
  ...
  impl Mapping {
    pub unsafe fn open(file: &File) -> io::Result<Self> {
      ...
      let ptr =
        unsafe { mmap(std::ptr::null_mut(), len, 1, 2, file.as_raw_fd(), 0) };
      if ptr as isize == -1 {
...
impl Table {
  ...
  fn add_slow(&mut self, data: &[u8], mut record: Record) -> io::Result<()> {
    ...
    record.signature =
      full_name_hash(&data[record.start..record.start + record.len]);
  ...
  fn make_slot(&mut self, data: &[u8], record: Record) -> io::Result<Slot> {
    ...
    std::str::from_utf8(name)
      .map_err(|_| invalid("station name is not UTF-8"))?;
    let mut tail = [0u8; 8];
    if name.len() > 8 {
...
const TEMPERATURE_LAYOUTS: [TemperatureLayout; 16] = {
  ...
  layouts[5] = TemperatureLayout {
    shuffle: [
      0x80, 0, 1, 2, 3, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80,
      0x80, 0x80,
    ],
  ...
  layouts[11] = TemperatureLayout {
    shuffle: [
      0, 1, 2, 3, 4, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80,
      0x80, 0x80,
    ],
  ...
  layouts[10] = TemperatureLayout {
    shuffle: [
      0x80, 1, 2, 3, 4, 0, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80,
      0x80, 0x80,
    ],
  ...
  layouts[6] = TemperatureLayout {
    shuffle: [
      1, 2, 3, 4, 5, 0, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80,
      0x80,
    ],
    pad: (6u64 << 48) | (1u64 << 63),
  };
...
fn parse_temp(word: u64) -> io::Result<(i16, usize)> {
  use std::arch::x86_64::{
    _mm_cvtsi128_si64, _mm_cvtsi64_si128, _mm_loadu_si128, _mm_shuffle_epi8,
    _pext_u64,
  };
...
unsafe fn read_record(data: &[u8], start: usize) -> io::Result<Record> {
  let len = data[start..start + 101]
    ...
    .ok_or_else(|| {
      invalid("station name exceeds 100 bytes or missing semicolon")
    })?;
  if len == 0 {
...
unsafe fn step(
  ...
  let tail = mask_name_word(
    unsafe { load_word(data, start + 8) },
    len.saturating_sub(8),
  );
  let (value, bytes) = parse_temp(unsafe { load_word(data, start + len + 1) })?;
  let next = start + len + 1 + bytes;
  let slot =
    unsafe { table.slots.get_unchecked_mut(bucket(key, TABLE_LEN - 1)) };
  if ((slot.signature ^ key)
    | (slot.tail ^ tail)
    | ((slot.name & 127) ^ len as u64))
    == 0
  {
...
unsafe fn slow_step(
  ...
  {
    let (value, bytes) =
      parse_temp(unsafe { load_word(data, start + len + 1) })?;
    slot.stats.add(value);
...
#[inline(always)]
unsafe fn parse_temperatures(
  words: [u64; 2],
) -> io::Result<([i64; 2], [u64; 2])> {
  use std::arch::x86_64::*;
  unsafe {
    ...
    let positive =
      _mm_srlv_epi64(input, _mm_and_si128(negative, _mm_set1_epi64x(8)));
...
fn pipeline_reader(
  ...
  if start > 0 {
    file.read_exact_at(
      std::slice::from_mut(&mut preceding),
      (start - 1) as u64,
    )?;
  }
  ...
  if !cancel.load(Ordering::Relaxed) {
    if let Some((mut buffer, base, len)) = previous {
      ...
      if tail > 0 {
        file.read_exact_at(
          &mut buffer[base + len..base + len + tail],
          end as u64,
        )?;
      }
      if work
...
fn mapped_window_worker<const STREAMS: usize>(
  ...
  loop {
    ...
    let mapped = unsafe {
      mapping::Mapping::region(file, map_start, map_end - map_start)
    }?;
  ...
  F: Fn() -> io::Result<Table> + Sync,
{
  ...
  let tables = if threads == 1 {
    ...
    std::thread::scope(|scope| -> io::Result<Vec<Table>> {
      ...
      for _ in 1..threads {
        handles
          .push(std::thread::Builder::new().spawn_scoped(scope, run_worker)?);
      }
...
fn main() -> io::Result<()> {
  ...
  let chunk_size =
    env_usize("FAST1BRC_CHUNK_KIB", default_chunk, 65536)? * 1024;
  ...
  let rows = match mode {
    ...
    InputMode::Mmap => {
      let window_size =
        env_usize("FAST1BRC_WINDOW_MIB", 16, 1024)? * 1024 * 1024;
...
mod file_tests {
  ...
  fn buffered_reuse_and_boundaries() {
    let path = std::env::temp_dir()
      .join(format!("fast1brc-buffer-test-{}", std::process::id()));
...
mod pipeline_tests {
  ...
  fn pipeline_errors_and_reader_boundaries() {
    let path = std::env::temp_dir()
      .join(format!("fast1brc-pipeline-test-{}", std::process::id()));
    ...
    for pass in 0..2 {
      for len in 1..=100 {
        for zeros in 0..2 {
          ...
          data.extend_from_slice(if pass == 0 {
            b";-99.9\n"
          } else {
            b";99.9\n"
          });
...
mod window_tests {
  ...
  fn independent_mapping_windows_preserve_boundary_rows_and_tails() {
    let path = std::env::temp_dir()
      .join(format!("fast1brc-window-test-{}", std::process::id()));
    let file = std::fs::OpenOptions::new()
      .read(true)
      .write(true)
      .create_new(true)
    ...
    for data in [&data[..], &data[..data.len() - 1]] {
      ...
      for streams in 1..=4 {
        for window in [257, 4096, 65536] {
          assert_eq!(
            output(
              &aggregate_windowed(&file, 4, streams, 113, window).unwrap()
            ),
            expected
          );
        }
  ...
  fn vector_temperatures_match_scalar_for_all_values_and_malformed_lanes() {
    for first in -999..=999 {
      let words = std::array::from_fn(|lane| {
        word((first + lane as i16 * 433 + 999) % 1999 - 999)
      });
      ...
      for lane in 0..2 {
        for position in 0..text.len() {
          for replacement in 0..=255 {
            ...
            if let (Ok((values, lengths)), Ok((value, len))) =
              (actual, expected)
            {
              assert_eq!(
                (values[lane], lengths[lane]),
