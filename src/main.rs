...
impl FxHasher64 {
    ...
    const fn add_to_hash(&mut self, word: u64) {
        self.hash = Self::hash_word(self.hash, word).wrapping_mul(FX_HASH_MULTIPLIER);
    }
}
...
fn process_worker(
    ...
    let mut local_map =
        HashMap::with_capacity_and_hasher(ESTIMATED_STATION_COUNT, BuildHasherDefault::default());
    ...
    for chunk_index in start_chunk..end_chunk {
        ...
        let read_size = std::cmp::min(chunk_size + prefix_len, file_size - read_start);
...
fn chunk_range(
    ...
    let start_chunk = chunks_per_worker
        ...
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "chunk range overflow"))?;
...
fn process_file_parallel(filename: &str) -> io::Result<StationMap> {
    ...
    let chunk_with_overlap = chunk_size
        .checked_add(CHUNK_OVERLAP)
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "chunk size overflow"))?;
    let chunk_with_overlap_usize = usize_from_u64(chunk_with_overlap, "chunk with overlap size")?;
    ...
    thread::scope(|scope| -> io::Result<()> {
        ...
        for thread_index in 0..num_threads {
            ...
            let (start_chunk, end_chunk) = chunk_range(total_chunks, num_threads, thread_index)?;
...
fn main() -> io::Result<()> {
    ...
    for (station_key, stats) in stations {
        let station = std::str::from_utf8(station_key.as_slice()).map_err(|error| {
            io::Error::new(
